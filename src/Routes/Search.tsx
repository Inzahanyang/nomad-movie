...
export default function Search() {
  ...
  const {
    ...
    isLoading,
    refetch: reMovie,
  } = useQuery<IGetMoviesResult>(["tv", "searchMovie"], () =>
    searchMovie(keyword!)
  );
  const { data: data2, refetch: reTv } = useQuery<IGetMoviesResult>(
    ["tv", "searchTv"],
    () => searchTv(keyword!)
  );
      ...
      {isLoading ? (
        ...
        <>
          <Banner onClick={incraseIndex}>
            ...
            <Overview>total results: {data?.total_results}</Overview>
          </Banner>
          <Slider>
                      ...
                      <BigCover
                        style={{
                          backgroundImage: `linear-gradient(to top, black, transparent), url(${makeImagePath(
                            clickedTvs.backdrop_path,
                            "w500"
                          )})`,
                        }}
                      />
                      <BigTitle>{clickedTvs.title}</BigTitle>
                      <BigOverview>{clickedTvs.overview}</BigOverview>
                    </>
                  )}
                </BigMovie>
              </>
